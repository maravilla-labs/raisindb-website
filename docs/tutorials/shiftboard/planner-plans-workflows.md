...
**What you'll have at the end of this part:** a second, plan-enabled agent that fills the whole weekend board in one request. It proposes a plan with one task per open shift, waits for the manager's approval, and then starts one durable fill-shift workflow (from Part 5) per task. The plan executes in seconds; the board fills asynchronously as staff accept their inbox tasks.

Parts 4 and 5 each solved one shift. The natural next request is "fill all open weekend shifts": five shifts, five workflows. You could prompt the chat agent to loop, but then the decomposition is invisible and ungated. You cannot see what it intends to do before it does it. The plan/task system addresses exactly that: the agent decomposes the request into a persisted, ordered task list, and an execution mode decides whether it runs immediately or waits for your approval.

This is the composition lesson of the tutorial: chat is the interface, plans are the gated intent, workflows are the durable process.
...
| `approve_then_auto` | One approval, then all tasks run. What this part uses |
| `step_by_step` | One task per "continue" signal |
| `manual` | No automatic task execution |

In the three gated modes `create-plan` stores the plan with status `pending_approval`; in `automatic` it starts `in_progress` right away.
...
Note the last rule: plans are for multi-step intent. A single-shift request does not pay the planning overhead; the agent calls the tool directly, exactly as in Part 5.
...
The frontend gains a `Board | Planner` toggle in the header. The Planner view keeps the shift board visible on the left, because the payoff of this part is watching the board fill, and swaps the right column for a plan panel and a chat with the coordinator. It is a second, independent conversation: the Board tab's chat with `/agents/shift-planner` is untouched.
...
## Two client-side timing details

Plan and task lifecycle messages are delivered into the conversation asynchronously. They are nodes being routed, not stream events. Two consequences show up in any real client, and `frontend/src/lib/stores/planner.svelte.ts` handles both with short reload polling (every 2 seconds):

1. **Proposal grace.** The turn's `done` event (finish reason `awaiting_plan_approval`) can arrive before the `ai_plan` card message lands in the conversation. If you treat `done` as "render final state", you briefly show an approval prompt for nothing. The fix: when the last finish reason is `awaiting_plan_approval` but the `plans` projection has no pending plan yet, reload the conversation until the card appears (the store gives up after about 20 seconds).
2. **Execution watch.** After `approvePlan()` the agent continues without any user message, so there is no stream attached to your next render. Poll `loadMessages()` until the plan reaches a terminal status (`completed`, `failed` or `cancelled`), skipping fetches while a live stream is active.
...
`planner-tab-check.mjs` drives the whole story end to end through a real browser and the API: reset all five shifts to open, log in as the manager, send "Fill all open weekend shifts", assert the proposal has exactly one task per open shift, approve, assert all tasks complete while the board is still all-open, assert Anna and Cara hold the workflows' inbox tasks, accept one via the API, and assert that shift flips to filled on the live page. It restores the demo state afterwards and asserts the run stayed within 15 model calls.

```bash
npm run planner-tab-check
```

It needs the built frontend running (`npm run build && npm run start` in `frontend/`), a Groq key, and Playwright installed in `PLAYWRIGHT_DIR` (default `/tmp/shots/node_modules`).

## Chat vs. plan vs. workflow

| | Chat tool calls (Part 2/4) | Plan (Part 6) | Workflow (Part 5) |
|---|---|---|---|
| Decomposition | Invisible, in the model's context | Visible, persisted task list | Modeled ahead of time |
...
You now have the complete stack: nodes as the single source of truth, an agent that acts through functions, a UI that is just subscriptions, plans that make multi-step intent visible and gated, and a workflow engine for everything that must not be forgotten.
